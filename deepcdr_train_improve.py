...
# Enable mixed precision for performance optimization if supported by the hardware
from tensorflow.keras import mixed_precision
mixed_precision.set_global_policy("mixed_float16") # Adjust based on hardware capabilities
...
def create_tf_dataset(generator_func, output_signature, *generator_args):
    """Wraps a generator function into a TensorFlow dataset.

    Args:
        generator_func (function): The data generator function.
        output_signature (tuple): A tuple of `tf.TensorSpec` defining dataset structure.
        *generator_args: Additional arguments to pass to the generator.

    Returns:
        tf.data.Dataset: A TensorFlow dataset with prefetching and type conversion.
    """
    ...
    def convert_structure(x, y):
        """Converts floating-point elements in x and y to tensors."""
        converted_x = [
    ...
    # Apply transformations: map conversion function and enable prefetching
...
def load_memmap_unknown_shape(directory, filename, mode='r'):
    ...
    Args:
        ...
        mode (str, optional): Mode in which to open the memory map. Defaults to 'r' (read-only).
...
def deepcdrgcn(dict_features, dict_adj_mat, samp_drug, samp_ach, cancer_dna_methy_model, cancer_gen_expr_model, cancer_gen_mut_model, training = False, dropout1 = 0.1, dropout2 = 0.2):
    """Builds the DeepCDR-GCN model for drug response prediction.

    Args:
        dict_features (dict): Dictionary containing drug feature matrices.
        dict_adj_mat (dict): Dictionary containing normalized adjacency matrices.
        samp_drug (str): Sample drug identifier.
        samp_ach (str): Sample cell line identifier.
        cancer_dna_methy_model (tf.keras.Model): Pre-trained model for DNA methylation.
        cancer_gen_expr_model (tf.keras.Model): Pre-trained model for gene expression.
        cancer_gen_mut_model (tf.keras.Model): Pre-trained model for gene mutation.
        training (bool, optional): If True, enables dropout layers during training. Defaults to False.
        dropout1 (float, optional): Dropout rate for general layers. Defaults to 0.1.
        dropout2 (float, optional): Dropout rate for fully connected layers. Defaults to 0.2.

    Returns:
        tf.keras.Model: A compiled DeepCDR-GCN model.
    """
    # Define GCN and generate graph-structured drug data
    ...
    # Combine omics and drug embeddings
...
def run(params: Dict):
    ...
    with strategy.scope():
        ...
        # Load drug data as memmap objects
        ...
        # Convert 'Cell_Line' to string and 'AUC' to float for model input compatibility
        ...
        # --------------------------------------------------------------------
        y_val_preds, y_val_true = batch_predict(check, data_generator(valid_gcn_feats, valid_adj_list, valid_keep["Cell_Line"].values.reshape(-1,1), valid_keep["Cell_Line"].values.reshape(-1,1), valid_keep["Cell_Line"].values.reshape(-1,1), valid_keep["AUC"].values.reshape(-1,1), generator_batch_size, verbose = False, shuffle = False), batch_size=generator_batch_size, steps=validation_steps)
